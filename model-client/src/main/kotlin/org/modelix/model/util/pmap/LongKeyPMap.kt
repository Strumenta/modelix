...
class LongKeyPMap<V> protected constructor(root: INode<V>?) {
    ...
    fun put(key: Long, value: V): LongKeyPMap<V?> {
        return LongKeyPMap<Any?>(this.root.put(key, value, 0))
    }

    fun remove(key: Long): LongKeyPMap<V?> {
        return LongKeyPMap<Any?>(root.remove(key, 0))
    }
    ...
    class InternalNode<V>(private val bitmap: Int, children: Array<INode<V>?>?) : INode<V?> {
        ...
        fun setChild(logicalIndex: Int, child: INode<*>?): INode<V>? {
            ...
            return if (isBitNotSet(bitmap, logicalIndex)) {
                InternalNode<V>(bitmap or (1 shl logicalIndex), COWArrays.insert<INode<*>>(children, physicalIndex, child))
            } else {
                InternalNode<V>(bitmap, COWArrays.set<INode<*>>(children, physicalIndex, child))
            }
        }
        ...
        companion object {
            private val EMPTY_CHILDREN = arrayOfNulls<INode<*>?>(0)
            val EMPTY: InternalNode<*> = InternalNode<Any?>(0, EMPTY_CHILDREN)
