...
class CLHamtInternal : CLHamtNode<CPHamtInternal?> {
    private override val data: CPHamtInternal
    ...
    override fun put(key: Long, value: String?, shift: Int): CLHamtNode<*>? {
        val childIndex = (key ushr shift and CLHamtNode.Companion.LEVEL_MASK.toLong()).toInt()
        val child = getChild(childIndex, NonBulkQuery(store))!!.execute()
        return if (child == null) {
            setChild(childIndex, CLHamtLeaf.Companion.create(key, value, store!!))
        } else {
            setChild(childIndex, child.put(key, value, shift + CLHamtNode.Companion.BITS_PER_LEVEL))
        }
    }
    ...
    override fun remove(key: Long, shift: Int): CLHamtNode<*>? {
        ...
        return if (child == null) {
            ...
            setChild(childIndex, child.remove(key, shift + CLHamtNode.Companion.BITS_PER_LEVEL))
        }
    }
    ...
    override fun get(key: Long, shift: Int, bulkQuery: IBulkQuery): IBulkQuery.Value<String?>? {
        ...
        return getChild(childIndex, bulkQuery)!!.mapBulk<String> { child: CLHamtNode<*>? ->
            ...
            child[key, shift + CLHamtNode.Companion.BITS_PER_LEVEL, bulkQuery]
        }
    }

    protected fun getChild(logicalIndex: Int, bulkQuery: IBulkQuery): IBulkQuery.Value<CLHamtNode<*>?>? {
        if (CLHamtNode.Companion.isBitNotSet(data.bitmap, logicalIndex)) {
            return bulkQuery.constant(null)
        }
        val physicalIndex: Int = CLHamtNode.Companion.logicalToPhysicalIndex(data.bitmap, logicalIndex)
    ...
    protected fun getChild(childHash: String?, bulkQuery: IBulkQuery): IBulkQuery.Value<CLHamtNode<*>?>? {
        return bulkQuery.get<CPHamtNode?>(childHash, CPHamtNode.Companion.DESERIALIZER).map { childData: CPHamtNode? -> CLHamtNode.Companion.create(childData, store) }
    }
    ...
    fun setChild(logicalIndex: Int, child: CLHamtNode<*>?): CLHamtNode<*>? {
        ...
        val physicalIndex: Int = CLHamtNode.Companion.logicalToPhysicalIndex(data.bitmap, logicalIndex)
        return if (LongKeyPMap.Companion.isBitNotSet(data.bitmap, logicalIndex)) {
    ...
    fun deleteChild(logicalIndex: Int): CLHamtNode<*>? {
        ...
        val physicalIndex: Int = LongKeyPMap.Companion.logicalToPhysicalIndex(data.bitmap, logicalIndex)
    ...
    override fun visitChanges(oldNode: CLHamtNode<*>, visitor: IChangeVisitor) {
        ...
        if (oldNode is CLHamtInternal) {
            ...
            if (data.bitmap == oldInternalNode.data.bitmap) {
                ...
                for (logicalIndex in 0 until CLHamtNode.Companion.ENTRIES_PER_LEVEL) {
