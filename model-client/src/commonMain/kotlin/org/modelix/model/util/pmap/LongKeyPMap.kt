...
import org.modelix.model.lazy.bitCount
import org.modelix.model.util.BiPredicate
import org.modelix.model.util.MutableObject
    ...
    class InternalNode<V>(private val bitmap: Int, children: Array<INode<V?>>) : INode<V?> {
        ...
        fun setChild(logicalIndex: Int, child: INode<*>?): INode<V?>? {
            ...
            return if (isBitNotSet(bitmap, logicalIndex)) {
                ...
                val resultingArray: Array<INode<V?>> = COWArrays.insert<INode<V?>>(children as Array<INode<V?>>, physicalIndex, child as INode<V?>) as Array<INode<V?>>
                InternalNode(bm, resultingArray)
            } else {
                InternalNode(bitmap, COWArrays.set<INode<V?>>(children, physicalIndex, child as INode<V?>))
            }
        }
        ...
        fun deleteChild(logicalIndex: Int): INode<V?>? {
            ...
            val newChildren: Array<INode<V?>> = COWArrays.removeAt(children, physicalIndex)
    ...
    class LeafNode<V>(val key: Long, val value: V) : INode<V?> {
        ...
        override fun visitChanges(oldNode: INode<V?>, visitor: IChangeVisitor<V?>) {
            ...
            if (oldValue.getValue() == null) {
                visitor.entryAdded(key, value)
            } else if (oldValue.getValue() !== value) {
                visitor.entryChanged(key, oldValue.getValue(), value)
            }
        }
